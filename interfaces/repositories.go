...
import (
	...
	"github.com/muhfaris/go-transaction/domain"
...
func (repo *DbItemRepo) Store(item domain.Item) {
	...
	VALUES ('%d', '%v', '%f', '%v')`, item.Id, item.Name, item.Value, available))
}

func (repo *DbItemRepo) FindById(id int) domain.Item {
	row := repo.dbHandler.Query(fmt.Sprintf(`SELECT name, value, avaiable
